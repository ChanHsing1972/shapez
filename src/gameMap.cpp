									...
									beltToPlace(12), minerToPlace(4), cutterToPlace(4), trashToPlace(4), currentTaskIndex(0)
{
	...
	// 连接按钮的信号和槽
	connect(beltButton, &QPushButton::clicked, this, [this]()
			{ onButtonClicked("Belt"); });
	connect(minerButton, &QPushButton::clicked, this, [this]()
			{ onButtonClicked("Miner"); });
	connect(cutterButton, &QPushButton::clicked, this, [this]()
			{ onButtonClicked("Cutter"); });
	connect(trashButton, &QPushButton::clicked, this, [this]()
...
void GameMap::mouseMoveEvent(QMouseEvent *event)
{
	...
	if (mouseX >= WINDOW_WIDTH)
		mouseX = WINDOW_WIDTH - 1;
	if (mouseY >= WINDOW_HEIGHT)
		mouseY = WINDOW_HEIGHT - 1;
	newMousePosition.setX(mouseX);
	newMousePosition.setY(mouseY);

	// 只在鼠标所在格子改变时更新，减少重绘次数
	if ((newMousePosition.x() / GRID_SIZE != mousePosition.x() / GRID_SIZE) || (newMousePosition.y() / GRID_SIZE != mousePosition.y() / GRID_SIZE))
	{
...
void GameMap::placeMinerAt(const QPoint &pos)
{
	...
	// 如果格子里不是矿物，则不能放置开采机
	if (!maps[0].cycleMines.contains(QPoint(pixelX / GRID_SIZE, pixelY / GRID_SIZE)) && !maps[0].rectMines.contains(QPoint(pixelX / GRID_SIZE, pixelY / GRID_SIZE)))
	{
		canPlaceMiner = false;
		rotationState = 0;
		return;
...
void GameMap::placeCutterAt(const QPoint &pos)
{
	...
	// 如果格子里有东西，则不能放置
	if (checkEmptyGrid(pixelX / GRID_SIZE, pixelY / GRID_SIZE) == false || ((rotationState == _W || rotationState == _S) && checkEmptyGrid(pixelX / GRID_SIZE + 1, pixelY / GRID_SIZE) == false) || ((rotationState == _D || rotationState == _A) && checkEmptyGrid(pixelX / GRID_SIZE, pixelY / GRID_SIZE + 1) == false))
	{
		canPlaceCutter = false;
		rotationState = 0;
		return;
...
void GameMap::onNewMineralGenerated(Mineral *mineral)
{
	...
	for (Belt *belt : beltList) // 遍历传送带列表，找到矿物所在的格子
	{
		...
		if ((QPoint(beltGridX, beltGridY - 1) == mineralGridPos && mineral->getDirection() == _S && (belt->getRotationState() == _S || belt->getRotationState() == _S_A || belt->getRotationState() == _S_D)) || (QPoint(beltGridX - 1, beltGridY) == mineralGridPos && mineral->getDirection() == _D && (belt->getRotationState() == _D || belt->getRotationState() == _D_S || belt->getRotationState() == _D_W)) || (QPoint(beltGridX, beltGridY + 1) == mineralGridPos && mineral->getDirection() == _W && (belt->getRotationState() == _W || belt->getRotationState() == _W_A || belt->getRotationState() == _W_D)) || (QPoint(beltGridX + 1, beltGridY) == mineralGridPos && mineral->getDirection() == _A && (belt->getRotationState() == _A || belt->getRotationState() == _A_S || belt->getRotationState() == _A_W)))
		{
			mineral->startMoving();
		}
	}
...
{
	if (currentMap.cycleMines.contains(QPoint(gridX, gridY)) || currentMap.rectMines.contains(QPoint(gridX, gridY)) || currentMap.barriers.contains(QPoint(gridX, gridY)) || currentMap.hubSmall.contains(QPoint(gridX, gridY)) || currentMap.devices[gridX][gridY] != nullptr)
	{
		return false;
	}
	return true;
...
void GameMap::createDeviceByTypeID(int typeID, int i, int j)
{
	...
	{
	case 0:
		placeBeltAt(QPoint(i * GRID_SIZE, j * GRID_SIZE));
		break;
	case 1:
		placeMinerAt(QPoint(i * GRID_SIZE, j * GRID_SIZE));
		break;
	case 2:
		placeCutterAt(QPoint(i * GRID_SIZE, j * GRID_SIZE));
		break;
	case 3:
		placeTrashAt(QPoint(i * GRID_SIZE, j * GRID_SIZE));
		break;
	default:
		break;
	}
